...
This repository contains a working prototype designed as a portfolio-ready demonstration, complete with mock service logic on the backend and an intuitive SaaS-inspired web interface.
...
- **Dual-Mode System**:
  - **API Mode**: Queries the ASP.NET Core server dynamically over CORS.
...
- [ ] **Real PDF Text Extraction**: Implement C# library parsing (e.g., `PdfPig` or `iTextSharp`) to extract raw text content on the backend.
- [ ] **AI-Powered Quiz Generation**: Integrate Gemini or OpenAI APIs to analyze extracted text and generate questions dynamically.
